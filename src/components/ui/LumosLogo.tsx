...
export const LumosLogo: React.FC<LumosLogoProps> = ({ className = '', size = 'md' }) => {
  return (
    <div
      className={`relative group/logo cursor-pointer flex items-center gap-2.5 sm:gap-3 select-none transition-transform duration-500 ease-out hover:scale-[1.025] ${className}`}
    >
      {/* 1. Soft Radial Golden Light Bloom Behind Logo (Reveals on Hover) */}
      <div
        className="absolute -inset-3 sm:-inset-4 rounded-2xl bg-radial from-[#F4D27A]/28 via-[#D9A83F]/12 to-transparent blur-xl opacity-0 group-hover/logo:opacity-100 transition-opacity duration-700 pointer-events-none"
        aria-hidden="true"
      />

      {/* 2. Golden Sunburst & Book Emblem */}
      <div className="relative">
        <svg
          className={`${
            size === 'sm' ? 'h-8 w-8' : size === 'lg' ? 'h-13 w-13' : 'h-10 w-10 sm:h-11 sm:w-11'
          } shrink-0 drop-shadow-[0_2px_10px_rgba(217,168,63,0.35)] group-hover/logo:drop-shadow-[0_0_16px_rgba(244,210,122,0.65)] group-hover/logo:brightness-115 transition-all duration-500`}
          viewBox="0 0 64 64"
          fill="none"
      ...
      {/* 3. Typography with Sheen Overflow Mask */}
      <div className="relative overflow-hidden flex flex-col justify-center text-left py-0.5">
        ...
        <span
          className="font-sans font-bold tracking-[0.24em] text-[#D9A83F] uppercase mt-1 drop-shadow-xs group-hover/logo:text-[#FFE7A3] transition-colors duration-500"
          style={{ fontSize: size === 'sm' ? '7.5px' : size === 'lg' ? '10.5px' : '8.5px' }}
        >
          TA‘LIM MARKAZI
        </span>

        {/* Light Sweep Sheen (600–900ms Left -> Right Sweep) */}
        <div
          className="absolute inset-0 -translate-x-[120%] group-hover/logo:translate-x-[160%] transition-transform duration-700 ease-out pointer-events-none bg-gradient-to-r from-transparent via-white/25 to-transparent skew-x-12"
          aria-hidden="true"
        />
      </div>
    </div>
